...
### M19 · FinOps : budget tronqué à 1000 events + observe-only (events morts) *(NOUVEAU)* — ✅ **cœur livré** (`1961e7e`)
> **✅ Livré (2026-06-21, fix-and-verify, mergé `main`)** : total budget **cumulatif** (agrégat SQL non borné) + test de régression (`pytest` 15 passed, vérifié local) + clarification honnête « budget observationnel / events réservés ». **Reste** : enforcement opt-in (décision design — mission future).
