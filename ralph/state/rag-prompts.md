...
| 4 | 2026-06-25 | GUARD-LEAK-PRECISION (détecteur direct) | `leaks_prompt_or_persona` (OWASP LLM01-02) testé via le RULE mais pas sa PRÉCISION. +3 tests directs : marqueur de persona/prompt AFFIRMÉ → fuite ; le même marqueur NIÉ (« je ne suis pas FreeBot ») → **PAS** une fuite (sinon on bloque des refus légitimes) ; marqueurs très spécifiques (owasp llm01) comptent PARTOUT, même en phrase niée (régurgitation interdite). Verrouille la frontière fuite réelle vs déni. | `tests/rag` **183 passed/64⏭** ; bandit 0 | (branche prod) |
| 5 | 2026-06-28 | T1: guardrail_postfilter devient ré-export (cycle1-securite) | La logique du garde-fou a été déplacée dans `actions/app/guardrail_core.py` (source unique). `tests/rag/guardrail_postfilter.py` est désormais un **ré-export pur** (`from app.guardrail_core import *` + nommés). `conftest.py` ajoute `_ACTIONS` au `sys.path` pour permettre l'import. Les 24 tests de `test_postfilter.py` restent **verts sans modification** (non-régressif). | `tests/rag/guardrail_postfilter.py` (ré-export) ; `tests/rag/conftest.py` (+path) ; `pytest tests/rag/test_postfilter.py` **24✅** | (branche prod/cycle1-securite) |
| 6 | 2026-06-28 | T1-FIX: ré-export auto-suffisant hors pytest | **Régression corrigée** (revue) : `run_live.py:38` et `live_harness.py:603` importent `guardrail_postfilter` comme **scripts autonomes** (hors pytest), où le `sys.path` n'est PAS amorcé par `conftest.py` → `ModuleNotFoundError: No module named 'app'`. Le ré-export ajoute désormais LUI-MÊME `actions/` au `sys.path` AVANT d'importer `app.guardrail_core` (bloc `os`/`sys` en tête, `# noqa: E402`). Le bloc dans `conftest.py` reste (redondant, inoffensif). Prouvé : `python -c "import guardrail_postfilter as g; print(g.post_filter(...).rule)"` depuis `tests/rag/` imprime `passthrough` sans erreur. | `tests/rag/guardrail_postfilter.py` (auto-path) ; import hors-pytest **OK** ; `pytest tests/rag/test_postfilter.py` **24✅** ; `pytest actions/tests` **222✅/5⏭** | (branche prod/cycle1-securite) |
