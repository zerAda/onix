...
| 5 | 2026-06-23 | BRANDING-DEMO | `nginx/onyx.conf` : 3 règles `sub_filter` supplémentaires rebrandent les libellés produit du login (« Welcome to Onyx »/tagline/« New to Onyx? » → GEREP). **Best-effort** (comme le `<title>`) : le texte rendu côté client par Next.js peut revenir à « Onyx » après hydratation — seul « New to Onyx? » (HTML statique) tient ; le rebranding fiable du **nom** = EE/whitelabel. Les **couleurs GEREP** (CSS `--theme-primary-*`) s'appliquent pleinement (vérifié par screenshot headless de la VM de démo). | Vérif visuelle : logo/bouton/liens en bleu GEREP #094785 ; libellé bas rebrandé | (branche prod, déployé VM démo) |
| 6 | 2026-06-23 | DEMO-SCRIPTS + DIAG-#12 | `scripts/demo/` : `demo-reconcile.sh` (réconciliation contrat↔SI Fabric) + `demo-rag.sh` (**RAG non-agentique** : récup → génération directe Ollama) + `README.md` (**diagnostic #12 Onyx 4.1.1↔Ollama** prouvé live : gemma3 sans tool-calling natif ; llama3.1 oui ; provider litellm `ollama`=prompt-based casse le tool-calling ; OpenAI-compat `/v1` débloque la **récupération** mais l'**étape réponse** agentique reste vide → parti pris RAG non-agentique, bonne archi modèles locaux). | Démo live : RAG `demo-rag.sh` répond grounded+cité (BETA 12 500 €/Prévoyance, GAMMA Santé/01-01-2026) ; réconciliation ECART/CONFORME | (branche prod, déployé VM démo) |
