...
| 5 | 2026-06-23 | #12-unwrap (durcissement) | `unwrap_wrapped_answer` gère désormais le JSON **encadré d'un bloc de code markdown** (clôture triple-backtick, avec tag de langage type `json`), comportement fréquent des modèles : helper `_strip_code_fence` (retire la clôture + le tag) avant le parse. Fail-safe préservé : bloc de code non-JSON ou texte ordinaire ⇒ inchangé. | gateway **353 passed** (+3) ; bandit 0 | (branche prod) |
| 6 | 2026-06-25 | sync-doc-acl (fail-closed mapping) | Couverture des chemins **fail-closed** du CLI `scripts/sync-doc-acl.py` (matérialise l'ACL SharePoint→`doc_acl.json`) : mapping **introuvable** OU **JSON corrompu** ⇒ code retour 2 et **aucun fichier écrit** (jamais de `doc_acl.json` vide/trompeur sur entrée absente). +2 tests (`capsys` prouve que l'échec vient du mapping, pas du contrôle Graph). | gateway **355 passed** (+2) ; bandit 0 | (branche prod) |
| 7 | 2026-06-25 | CONFIG-ENUMS (fail-closed) | `config.py` lisait `GATEWAY_GROUP_SOURCE` et `GATEWAY_DOC_ACL_DEFAULT_POLICY` **sans validation** → un typo (`grpah`) basculait **silencieusement** vers le repli `auto`/`deny` au lieu du comportement voulu (anti-pattern vs posture fail-closed). Helper `_validate_choice` : valeur hors liste ⇒ **refus de démarrage** avec message clair ; valeurs valides normalisées casse/espaces. Nouveau `tests/test_config.py`. | gateway **366 passed** (+11) ; bandit 0 | (branche prod) |
