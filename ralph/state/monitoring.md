...
| 9 | 2026-06-25 | BLACKBOX-CROSSCHECK (sondes) | Extension de `test_prometheus_config.py` (+2 tests) : chaque module blackbox a un `prober` ; et surtout **cross-check** — tout module référencé par un job Prometheus (`params.module`, ex. `http_2xx`) DOIT être défini dans `blackbox.yml`, sinon la sonde casse (« module not found ») et les alertes de disponibilité (ProbeFailed/ActionsServiceDown, basées sur `probe_success`) deviennent inertes EN SILENCE. | `pytest scripts/tests/test_prometheus_config.py` **6✅** (.venv-gw) ; bandit 0 | (branche prod) |
