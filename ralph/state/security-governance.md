...
| S2 | P1 | Hook gitleaks pre-commit annoncé mais inexistant (`docs/SECURITY.md:67`) | A3/A6 | ✅ (`.pre-commit-config.yaml` posé par l'agent principal) |
...
- (aucune) — S2 résolu : `.pre-commit-config.yaml` créé par l'agent principal
  (hook gitleaks pin `v8.18.2` + hooks d'hygiène ; YAML validé, 10 `.json` valides,
  gitleaks 0). `docs/SECURITY.md` mis à jour (« Fourni » au lieu de « recommandé »).
...
- [x] A6 (gate gitleaks vert ; hook pre-commit `.pre-commit-config.yaml` posé)
