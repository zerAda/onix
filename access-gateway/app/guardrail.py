"""guardrail — post-filtre déterministe « couche 3 », DÉPLOYÉ dans la passerelle.

Ce module est le **portage applicatif** de la logique déterministe prouvée en T1
dans `tests/rag/guardrail_postfilter.py`. En T1, le post-filtre tournait dans le
**harnais de test** (à côté de l'appel LLM) pour PROUVER qu'il rattrapait les
relâchements résiduels d'un modèle ≥ 7B. Ici il devient un **contrôle réellement
déployé** : la passerelle l'exécute sur la **réponse de l'assistant Onyx**, dans
le chemin de service, AVANT de la renvoyer à l'utilisateur.

Pourquoi câbler ici (la gateway) et pas ailleurs :
  * La passerelle est **déjà devant Onyx** et **intercepte la réponse** de
    l'assistant (chemin réponse de `/v1/chat/send-message`). C'est le **dernier
    point sous notre contrôle** avant l'utilisateur — le bon endroit pour un
    garde-fou de sortie.
  * Le post-filtre s'exécute **hors-LLM**, **après** le LLM : une injection (même
    réussie côté modèle) ne peut pas le désactiver (elle n'a aucune prise sur du
    code déterministe Python). C'est la recommandation OWASP LLM01 (contrôle
    déterministe non manipulable).
  * Le RBAC (filtre Document Set forcé) reste **intact** sur le chemin requête ;
    ce module n'agit QUE sur le chemin réponse — défense en profondeur.

Propriétés : **déterministe**, **stateless**, **conservateur** (au moindre doute
de violation → substitution d'un refus sourcé). Un faux refus est inoffensif
(l'utilisateur reformule) ; une fuite ne l'est pas.

NB : ce module est volontairement SANS dépendance (stdlib uniquement) pour rester
embarquable dans l'image de la passerelle sans alourdir `requirements.txt`.
"""
