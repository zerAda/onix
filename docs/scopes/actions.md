...
| [`app/agentic_local.py`](../../actions/app/agentic_local.py) | **Couche agentique souveraine** (tool-calling natif Ollama `/api/chat`, contourne le mur #12) : `run_agent` STATELESS borné, `REGISTRY` d'outils **lecture-seule** whitelistés (client_360, portfolio_360, reconcile_batch, rag_ask, list_tasks, audit ; compartiment write vide), défense prompt-injection 6 couches (whitelist, scan L2 `_scan_injection` — **best-effort par signatures EN/FR**, le garde-fou L3 `guardrail_core` en sortie est le filet déterministe pour les injections sans marqueur connu —, confinement egress, audit), gate+audit par appel, sérialisation `default=str` (fail-closed, jamais de 500). Exposé par `POST /agent/ask` (gate `agent`, fail-closed). |
