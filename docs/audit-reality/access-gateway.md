...
| RBAC-35 | **[M3]** Pré-résolution `build_fabric_acl` : par doc du mapping → garde GOLD (`item_in_gold_scope`) puis roleAssignments du workspace → `_Entry(groups,users)` ; **deny-by-default** (doc non mappé / hors gold / roleAssignments illisibles ⇒ OMIS) | ✅ | `app/fabric_doc_acl.py:164-237` ; gold gate `:204-211` ; fail-closed roleAssignments `:212-228` | roleAssignments mémoïsés par workspace (1 lecture). Tests : `test_fabric_doc_acl.py::{test_citation_fabric_autorisee_conservee, test_fabric_doc_acl_deny_quand_non_mappe}`. |
| RBAC-36 | **[M3]** Variables `GATEWAY_DOC_ACL_FABRIC_ENABLED` (false, opt-in), `_MAPPING_PATH` (`config/fabric_acl.json`) ; mapping absent ⇒ ACL Fabric **vide** (deny total) ; Fabric non configuré ⇒ ACL vide, **aucun appel réseau** | ✅ | `config.py:149-153,301-304` ; `fabric_doc_acl.py:181-184` (vide si non configuré) ; `load_mapping :239-262` (absent → `{}`) | Testé `test_fabric_doc_acl.py::test_build_fabric_acl_vide_si_non_configure` (0 appel réseau). |
| RBAC-37 | **[#12 stopgap CPU]** RAG **non-agentique** : la gateway force l'outil `internal_search` d'Onyx (`forced_tool_id`+`allowed_tool_ids`) sur le payload chat relayé (stream + non-stream) ⇒ Onyx exécute la recherche (`tool_choice=REQUIRED`) ⇒ réponse **sourcée + citée** même avec un modèle CPU faible. **Prouvé live** (gemma3:12b, token grounded + citation, cf. RUNTIME-EVIDENCE #12). No-op si le client a déjà choisi ses outils, ou si `GATEWAY_FORCE_INTERNAL_SEARCH=false` (modèle agentique fiable/GPU). | ✅ | `app/onyx_proxy.py` (`force_internal_search`) ; `app/main.py` (appel après `enforce_document_sets`, couvre 2 chemins) ; `config.py` (`force_internal_search`,`force_search_tool_id`) | `tests/test_onyx_proxy.py` (4 tests : injection, tool_id configurable, no-op désactivé, respect choix client) |
| RBAC-38 | **[defense-in-depth API-compat]** Le périmètre RBAC Document Set est posé sur les **deux** champs de filtre : `retrieval_options.filters.document_set` (ancien schéma) ET `internal_search_filters.document_set` (SendMessageRequest Onyx 4.1.x). Ajouter un filtre ne peut que **resserrer** (jamais élargir) ⇒ strictement sûr quelle que soit la version honorée. *(Quelle version 4.1.1 honore réellement = à confirmer live — finding tracé RUNTIME-EVIDENCE #12.)* | ✅ | `app/onyx_proxy.py:enforce_document_sets` | `tests/test_onyx_proxy.py::{test_perimetre_pose_sur_les_deux_champs_filtres, test_internal_search_filters_client_ne_peut_pas_elargir}` |
| RAG-UNWRAP | **[#12 stopgap]** Déballage **défensif** d'une réponse JSON-enveloppée de gemma3 (`{"id":...,"result":"<texte>"}`) sur le chemin réponse, AVANT garde-fous/ACL. Ne déballe QUE si objet JSON avec `result` str ; sinon texte INCHANGÉ (cas normal, citations `[[1]]`, grounding préservés). | ✅ | `app/onyx_proxy.py:unwrap_wrapped_answer` (câblé dans `extract_answer`, 3 points de retour) | `tests/test_onyx_proxy.py` (5 tests : extraction result / texte normal / json sans result / result non-str / json invalide) |
