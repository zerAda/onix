...
| Toute modif/suppression/réordonnancement détectable ; `GET /admin/audit/verify` | ✅ | `audit_log.py:165-211`, `main.py:872-876` | testé altération → `broken_at` |
| **[M1] Anti-downgrade fail-closed** : clé présente ⇒ chaîne 100 % `hmac-sha256` ; toute ligne `sha256` = downgrade keyless (recalculable sans clé) ⇒ `ok:false`, reason « downgrade ». Pas de clé : best-effort `sha256`, ligne `hmac-sha256` = invérifiable ⇒ rupture. L'algo stocké par ligne NE pilote PLUS la vérif. | ✅ | `audit_log.py:187-207` | `tests/test_audit_log.py::test_verify_chain_detecte_downgrade_keyless_quand_cle_presente` + non-régression hmac/keyless ; `tests/test_security_rgpd.py::test_audit_chain_sans_cle_puis_avec_cle_signale_downgrade` |
| Secret absent → repli **SHA-256** (avertissement), à définir en prod | ✅ | `audit_log.py:88-103,139-145` | repli best-effort ; migration keyless→HMAC ⇒ base d'audit vierge (mélange = downgrade refusé, cf. M1) |
| **[HARD-03] Préflight fail-closed** : sans `ONIX_ACTIONS_AUDIT_HMAC_KEY`, le service **refuse de démarrer** (sauf override DEV `ONIX_ACTIONS_AUDIT_KEY_OPTIONAL=true`) — un journal keyless est forgeable, donc pas « inviolable ». | ✅ | `audit_log.py` (`preflight_audit_key`) ; `main.py:_lifespan` (appelé au démarrage) | `tests/test_security_rgpd.py::{test_preflight_audit_key_fail_closed_sans_cle, test_preflight_audit_key_ok_avec_cle}` ; suite **92 passed** |
