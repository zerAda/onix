...
| Backup/rollback : `make backup` (**arrêt bref + tar à froid des volumes**, **pas** de `pg_dump` logique), `make restore DIR=…` (non testée) | ❌ | `scripts/backup.sh:49-55` ; `scripts/restore.sh` | **Corrigé 2026-06-21** : la claim « dump à chaud `pg_dump` » était fausse — `backup.sh` fait un `tar czf` du volume `db_volume` brut après `stop`, pas un dump logique ; archives non chiffrées ; restauration sans gate santé. Cf. backlog M5/M6. |
